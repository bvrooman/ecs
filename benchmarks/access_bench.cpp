...
int main() {
    ...
    // query iteration: 1 archetype vs the same rows spread over 64
    {
        ...
        double const one = bench::min_ns_per(N, R, [&] {
            w.for_each_chunk<P const>([&](std::span<Entity const>, chunk<P const> p) {
        ...
        double const many = bench::min_ns_per(N, R, [&] {
            frag.for_each_chunk<P const>([&](std::span<Entity const>, chunk<P const> p) {
                for (float x : p.column<0>())
                    s64 += x;
            });
            bench::keep(s64);
        ...
        bench::emit("fragmentation_overhead",
                    "ratio",
                    many / one,
                    "x",
                    "lower",
