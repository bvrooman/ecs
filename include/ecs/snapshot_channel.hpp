...
public:
  SnapshotChannel() : writing_(std::make_shared<T>()) {}
  explicit SnapshotChannel(const T& init)
  ...
  void publish() {
    latest_.store(std::shared_ptr<const T>(writing_), std::memory_order_release);
    writing_ = std::make_shared<T>();
  }

  // --- consumer side (any number of threads) ----------------------------
  // The most recently published snapshot, or nullptr if nothing published yet.
  // The returned shared_ptr keeps that snapshot valid for as long as held.
  std::shared_ptr<const T> latest() const {
    return latest_.load(std::memory_order_acquire);
  }

  // Per-consumer cursor that advances only when a newer snapshot is available,
  // so a consumer can cheaply tell "is there new data?" and avoid reprocessing.
  class Reader {
  public:
    explicit Reader(const SnapshotChannel& channel) : channel_(&channel) {}
    ...
    const T* get() const noexcept { return current_.get(); }
    const std::shared_ptr<const T>& snapshot() const noexcept {
      return current_;
    }
...
private:
  ...
  std::atomic<std::shared_ptr<const T>> latest_; // last published (immutable)
};
