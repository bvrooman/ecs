...
int main() {
    ...
    std::thread sim([&] {
        ...
        while (!stop.load(std::memory_order_acquire)) {
            schedule.run(world, scheduler); // emits, simulates, publishes
    ...
    std::thread render([&] {
        ...
        while (!stop.load(std::memory_order_acquire)) {
            ...
            // Grab one frame after ~2s, once the fountain has reached steady state.
            if (capPath && !captured && frames >= 120 && count > 0) {
