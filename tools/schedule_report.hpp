...
//
// A ScheduleObserver that reports rolling timing distributions: for the whole
...
// in execution order. Attach it with Schedule::add_observer():
//
...
// two timing observers the profiler was split into (the other is
// ScheduleTrace); register either, both, or neither.
//
// Not gated by any macro -- like the core observer mechanism it builds on
// (Schedule::add_observer), this tool is always available. Whether to attach it
// is the application's choice, typically at runtime; the mist example, for one,
// constructs it only when the ECS_STATS env var is set.
...
// it composes with other observers without coordinating with them.
class ScheduleReport : public ecs::ScheduleObserver {
public:
    explicit ScheduleReport(Sink sink, double report_every_s = 2.0)
        : sink_(std::move(sink))
        , tick_(report_every_s) {}

    void tick_begin(std::size_t n_waves) override {
        if (waves_.size() < n_waves) {
            waves_.resize(n_waves);
            wave_systems_.resize(n_waves);
        }
        // Rebuild the per-wave system ordering each tick so it always reflects the
        // current execution order (cheap: a few small vectors, capacity retained).
        for (auto& ids : wave_systems_)
            ids.clear();
        t_tick_ = clock::now();
    }
    void wave_begin(std::size_t level, std::size_t) override {
        cur_wave_ = level;
        t_wave_   = clock::now();
    }
    void system_begin(SystemId id, std::string_view name) override {
        cur_ = &slot(id, name);
        wave_systems_[cur_wave_].push_back(id); // execution order within the wave
        t_sys_ = clock::now();
    }
    void system_end(SystemId) override { cur_->stats.sample(elapsed_us(t_sys_)); }
    void wave_end(std::size_t level) override {
        waves_[level].sample(elapsed_us(t_wave_));
    }
