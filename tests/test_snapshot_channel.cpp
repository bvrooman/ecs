  ...
  for (int c = 0; c < kConsumers; ++c) {
    consumers.emplace_back([&, c] {
      ...
      while (!done.load(std::memory_order_acquire) || r.poll()) {
        ...
        const int seq = f.seq;
        for (int v : f.data)
          if (v != seq) ++torn;     // immutable snapshot => must be consistent
        if (seq < last) ++torn;     // published order preserved
        last = seq;
      }
...
static void world_extraction_fans_out_to_two_consumers() {
  World w;
  for (int i = 0; i < 500; ++i) w.create_with(Position{float(i), 0});
  ...
  auto consume = [&](int id) {
    ...
    while (!stop.load(std::memory_order_acquire)) {
      if (r.poll()) seen[id].store(int(r.get()->size()), std::memory_order_relaxed);
    }
    if (r.poll() && r.get())
